	...
	private class TimedPayment {
		...
		public Boolean tick() {
			if (next == 0) {
				...
				Integer amountStart = amount.getAmount();
				Currency currencyStart = amount.getCurrency();
				Currency currencyFinish = tobank.getCurrency();
				Money value = new Money(currencyStart.valueInThisCurrency(amountStart, currencyFinish), currencyFinish);

				fromaccount.withdraw(amount);
				try {
					tobank.deposit(toaccount, value);
				}
